...
from model_beam_search import Model
import time
...
class Trainer(object):
    ...
    def __init__(self, data_params):
        ...
        self.model = Model(data_params, self.embedding)
        ...
        for i_epoch in range(self.data_params['max_epoches']):
            ...
            for _ in range(num_per_epoch):
                ...
                batch_data ={
                    ...
                    self.model.decode_input: response_vecs,
                    self.model.target: target_idx,
                    self.model.mask: target_mask,
                    self.model.is_training: True
                }

                indics = sess.run( self.model.training_pred, feed_dict = batch_data)
                
                sent_list = list()
                for idxs in indics:
                    sent = [self.train_batcher.idx_to_word[idx] for idx in idxs if idx != 20001]
                    if len(sent) < 3 :
                        sent = ['thanks','__eou__','__eou__']
                    sent = ' '.join(sent)
                ...
                _, loss, summary, g_step, seq_mask= sess.run([self.model.train_proc, self.model.loss,
                                                    self.model.summary_proc, self.model.global_step, self.model.seq_mask], feed_dict = batch_data)
            ...
            if i_epoch % self.data_params['evaluate_interval'] == 0:
                ...
                print ('****************************')
                print ('Epoch %d ends. Average loss %.3f. %.3f seconds/epoch' % (i_epoch, avg_batch_loss, t_end-t_begin))
                valid_acc = self._evaluate(sess, self.model, self.valid_batcher, self.data_params['ground_valid'], self.data_params['output_dir']+'valid'+str(i_epoch)+'.txt')
                print ('****************************')
    ...
    def _test(self, sess, epoch):
        ...
        valid_acc = self._evaluate(sess, self.model, self.valid_batcher, self.data_params['ground_valid'],  self.data_params['output_dir']+'valid'+str(epoch)+'.txt')
        print ('Test set:')
        test_acc = self._evaluate(sess, self.model, self.test_batcher, self.data_params['ground_test'],  self.data_params['output_dir']+'test'+str(epoch)+'.txt')
    ...
    def _evaluate(self, sess, model, batcher, ground_file, result_file):
        ...
        for _ in range(num_per_epoch):
            ...
            beam_output = sess.run( model.beam_output, feed_dict = batch_data)

            for idxs in beam_output:
                print(idxs)
                sent = [batcher.idx_to_word[idx[4]] for idx in idxs if idx[4] != 20001 and idx[4] != -1]
                if len(sent) < 3:
                    sent = ['thanks','__eou__','__eou__']
                sent = ' '.join(sent)
                sent_list.append(sent)
            # for idxs in target_idx:
            #     sent = [batcher.idx_to_word[idx] for idx in idxs]
            #     sent = ' '.join(sent)
            #     org_sents.append(sent)
        sents = '\n'.join(sent_list)
