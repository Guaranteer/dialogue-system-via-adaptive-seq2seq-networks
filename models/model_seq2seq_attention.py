...
class Model(object):
    ...
    def build_train_proc(self):
        ...
        self.sent_features = sent_outputs
        ...
        with tf.variable_scope('linear'):
            decoder_input_W = tf.get_variable('sw', shape=[self.lstm_dim, self.decode_dim], dtype=tf.float32,
                                              initializer=tf.contrib.layers.xavier_initializer())
            decoder_input_b = tf.get_variable('sb', shape=[self.decode_dim], dtype=tf.float32,
                                              initializer=tf.contrib.layers.xavier_initializer())

            self.decoder_input = tf.matmul(sent_last_state, decoder_input_W) + decoder_input_b
        ...
        with tf.variable_scope("decoder"):
            ...
            with tf.variable_scope("decoder_lstm") as scope:
                for i in range(self.max_r_words):
                    ...
                    # decoder : GRU
                    decoder_input = tf.concat([decoder_state, s_attention_decoder, current_emb], axis=1)
                    ...
                    decoder_middle = tf.concat([tf.multiply(decoder_r_t, decoder_state), tf.multiply(decoder_r_t, s_attention_decoder), current_emb], axis=1)
